...
def connect_sheet():
    creds_json = json.loads(st.secrets["GOOGLE_CREDS"])
    ...
    SHEET_URL = "https://docs.google.com/spreadsheets/d/1OT-Pjf0W8KP9QthrucmRF8HAtdOyJmn38kZ8WWAAOp4/edit?gid=0#gid=0"
        ...
        if (btn_predict_file or btn_anom_file) and file is not None:
            ...
            if btn_anom_file:
                ...
                if st.button("📮 Gửi dữ liệu file cho Admin"):
                    id_sent = push_request_to_admin(
                        user_data=df_input.to_dict(orient="records"),
                        model_data=out_view.to_dict(orient="records"),
                        source="file"
                    )
